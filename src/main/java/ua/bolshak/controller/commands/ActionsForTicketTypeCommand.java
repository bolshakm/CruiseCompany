...
public class ActionsForTicketTypeCommand implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (action.equals(UPDATE)){
            page = new ToUpdateTicketTypeCommand().execute(request, response);
        }
        if (action.equals(DELETE)){
            page = new DeleteTicketTypeCommand().execute(request, response);
        }
        return page;
    }
}
