...
public class ActionsForBonusesCommand implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (actionUpdate != null){
            page = new UpdateBonusCommand().execute(request, response);
        }
        if (actionDelete != null){
            page = new DeleteBonusCommand().execute(request, response);
        }
        return page;
    }
}
