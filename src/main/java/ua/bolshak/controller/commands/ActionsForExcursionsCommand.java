...
public class ActionsForExcursionsCommand implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (action.equals(UPDATE)){
            page = new UpdateExcursionCommand().execute(request, response);
        }
        if (action.equals(DELETE)){
            page = new DeleteExcursionCommand().execute(request, response);
        }
        return page;
    }
}
