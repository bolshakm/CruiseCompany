...
public class ActionsForUserCommand implements ICommand{
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (actionUpdate != null){
            page = new ToUpdateUserCardCommand().execute(request, response);
        }
        if (actionDelete != null){
            page = new DeleteUserCommand().execute(request, response);
        }
        return page;
    }
}
