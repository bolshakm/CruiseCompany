...
public class ActionsForCruiseStatusCommand implements ICommand {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (action.equals(UPDATE)){
            page = new UpdateCruiseStatusCommand().execute(request, response);
        }
        if (action.equals(DELETE)){
            page = new DeleteCruiseStatusCommand().execute(request, response);
        }
        return page;
    }
}
